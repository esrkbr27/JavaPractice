...
public class C17_NotOrtalama {
    public static void main(String[] args) {
        ...
        */
        ...
        //isimlerin indexleri ile notların indexleri,proje notların indexleri aynı kabul ediliyor.
        double[] ortalamaNotu = new double[10];
        for (int i = 0; i < 10; i++) {
            ortalamaNotu[i] = vizeFinalNotu[i][0] * 0.3 + vizeFinalNotu[i][1] * 0.5 + projeNotu[i] * 0.2;
        }
        //  System.out.println(Arrays.toString(ortalamaNotu));
       /* Örnek : JAVA 82 SINIFININ NOT LİSTESİ
        ADI      V   F    P    ORT
        Adem    100 100 100 100.0
        Emsal    50 50 50 50.0  gibi*/
        System.out.println("JAVA 82 SINIFININ NOT LISTESI\n" +
                "   ADI      V   F    P    ORT");
        for (int i = 0; i < 10; i++) {
            System.out.println(i + 1 + ". " + isimler[i] + "   " + vizeFinalNotu[i][0] + "   " + vizeFinalNotu[i][1] + "     " + projeNotu[i]
                    + "    " + ortalamaNotu[i]);
        }
        // III - Sınıfın Ağırlıklı not ortalamasını bulun ve yazdırın; ( Tüm öğrencilerin ortalamasının,
        //öğrenci sayısına bölünmesi ile bulunur. )
        double sum = 0;
        for (int i = 0; i < 10; i++) {
            sum += ortalamaNotu[i];
        }
        sum = sum / 10;
        System.out.println("sinifin agirlikli ortalamasi: " + sum);
        /*
        IV - Ağırlıklı Sınıf Not Ortalamasından daha düşük not ortalamasına sahip kişileri
        BÜTÜNLEMEYE GELECEK ÖĞRENCİLER şeklinde isimleri ve karşılarına not ortalamaları gelecek şekilde
        Örnek : BÜTÜNLEMEYE KALAN ÖĞRENCİLER
        ...
        Hüseyin               55.26
        Hatice                61.25   gibi*/
        System.out.println("BUTUNLEMEYE KALAN OGRENCILER\n Adi                 Ortalamasi");
        for (int i = 0; i < 10; i++) {
            if (ortalamaNotu[i] < sum) {
